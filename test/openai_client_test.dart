  ...
  test('OpenAI-compatible client lists models and parses tool calls', () async {
    ...
    server.listen((request) async {
      ...
      if (request.uri.path == '/v1/models') {
        request.response.write(jsonEncode({'data': [{'id': 'mock-model'}]}));
                ...
                'tool_calls': [
                  {
                    'id': 'call_1',
                    'type': 'function',
                    'function': {'name': 'web_search', 'arguments': '{"query":"Tavo"}'}
                  }
                ]
              }
            }
    ...
    final client = OpenAiCompatibleClient(ApiProviderConfig(
      name: 'mock',
