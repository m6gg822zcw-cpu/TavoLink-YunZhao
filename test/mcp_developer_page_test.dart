...
class _Client extends McpClient {
  ...
  Future<McpConnectResult> connect() async =>
      const McpConnectResult(protocolVersion: 'fixture', serverName: 'Fixture');

  @override
  Future<List<McpTool>> listTools() async => const [
    McpTool(
      name: 'fixture.echo',
      inputSchema: {
        'type': 'object',
...
void main() {
  testWidgets(
    'loads saved token, requires confirmation and clears client on exit',
    (tester) async {
      late _Client client;
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: McpDeveloperPage(
  ...
  testWidgets('rejects non-object JSON without sending a tool call', (
    tester,
  ) async {
    late _Client client;
    await tester.pumpWidget(
