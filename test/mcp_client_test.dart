...
void main() {
  test('MCP 2026 stateless connect lists and calls tools', () async {
    ...
    server.listen((request) async {
      ...
      if (method == 'tools/list') {
        request.response.write(jsonEncode({'jsonrpc': '2.0', 'id': body['id'], 'result': {'tools': [
          {'name': 'tavo.echo', 'description': 'Echo', 'inputSchema': {'type': 'object'}}
        ]}}));
      } else if (method == 'tools/call') {
        request.response.write(jsonEncode({'jsonrpc': '2.0', 'id': body['id'], 'result': {'content': 'ok'}}));
      }
      await request.response.close();
    });

    final client = McpClient(McpServerConfig(name: 'Mock Tavo', url: Uri.parse('http://127.0.0.1:${server.port}/mcp')));
  ...
  test('auto mode falls back to direct JSON-RPC', () async {
    ...
    final client = McpClient(McpServerConfig(name: 'Tavo Direct', url: Uri.parse('http://127.0.0.1:${server.port}/mcp')));
    final connection = await client.connect();
