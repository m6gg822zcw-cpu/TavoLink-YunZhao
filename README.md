...
工程固定使用 Flutter `3.47.2`，Android 与 iOS 原生工程已提交到仓库：
...
`tool/bootstrap_platforms.sh` 只用于原生目录意外缺失时恢复 scaffold；它不会覆盖已提交的原生配置。`tool/patch_platforms.py` 可重复应用 Android Internet / 局域网策略与 iOS Local Network / ATS 本地网络说明。
...
- Ubuntu runner：`flutter analyze` + `flutter test` + 三套契约测试 + `flutter build apk --release`，上传 `TavoLink-YunZhao-1.1.0.apk`。
- macOS runner：完成测试后执行 iOS Release no-codesign 构建，并打包 `TavoLink-YunZhao-1.1.0-unsigned.ipa`。
- 推送 `v*` Tag 时会把 APK、IPA、SHA-256 与 Android 签名模式说明发布到 GitHub Release。
- 配置 Android / Apple 签名 Secrets 后，同一工作流会改用正式 Android upload key，并额外生成签名 IPA。
