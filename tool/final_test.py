...
def sha256_file(path):
    ...
    with path.open('rb') as source:
        for chunk in iter(lambda: source.read(1024 * 1024), b''):
            digest.update(chunk)
    path.with_name(path.name + '.sha256').write_text(f'{digest.hexdigest()}  {path.name}\n')
...
if flutter:
    run('Locked Flutter dependencies', [flutter, '--suppress-analytics', '--no-version-check', 'pub', 'get', '--enforce-lockfile'], timeout=300)
    run('Flutter analyze', [flutter, '--suppress-analytics', '--no-version-check', 'analyze', '--fatal-infos', '--no-pub'], timeout=300)
    run('Flutter tests', [flutter, '--suppress-analytics', '--no-version-check', 'test', '--no-pub'], timeout=300)
    ok = run('Android release APK build', [flutter, '--suppress-analytics', '--no-version-check', 'build', 'apk', '--release', '--no-pub'], timeout=1200)
    if ok:
        apk = ROOT/'build/app/outputs/flutter-apk/app-release.apk'
        if apk.exists():
            target = DIST/'TavoLink-YunZhao-1.1.0.apk'
...
if platform.system() == 'Darwin' and flutter and xcode:
    ok = run('iOS release no-codesign build', [flutter, '--suppress-analytics', '--no-version-check', 'build', 'ios', '--release', '--no-codesign', '--no-pub'], timeout=1200)
    if ok:
        ...
        if app.exists():
            ...
            ipa = DIST/'TavoLink-YunZhao-1.1.0-unsigned.ipa'
