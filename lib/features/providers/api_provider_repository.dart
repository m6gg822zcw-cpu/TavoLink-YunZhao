...
class ApiProviderRepository {
  const ApiProviderRepository({SecureStore secureStore = const SecureStore()}) : _secureStore = secureStore;
  ...
  Future<ApiProviderConfig?> load() async {
    ...
    if (rawUrl == null || rawUrl.trim().isEmpty || model == null || model.trim().isEmpty) return null;
    final uri = Uri.tryParse(rawUrl);
    if (uri == null || !uri.hasScheme || !uri.hasAuthority) return null;
    final rawHeaders = await prefs.getString(_headersKey);
    final headers = rawHeaders == null || rawHeaders.isEmpty
        ? <String, String>{}
        : (jsonDecode(rawHeaders) as Map).map((key, value) => MapEntry(key.toString(), value.toString()));
