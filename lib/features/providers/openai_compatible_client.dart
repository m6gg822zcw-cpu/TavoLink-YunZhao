...
class OpenAiCompatibleClient {
  ...
  Map<String, String> get _headers => {
        ...
        if (config.apiKey?.isNotEmpty == true) 'Authorization': 'Bearer ${config.apiKey}',
        ...config.extraHeaders,
      };
  ...
  Future<List<String>> listModels() async {
    final response = await _dio.getUri(
      _endpoint('models'),
      options: Options(headers: _headers, sendTimeout: const Duration(seconds: 20), receiveTimeout: const Duration(seconds: 20)),
    );
    ...
    return models.whereType<Map>().map((m) => m['id']?.toString()).whereType<String>().where((e) => e.isNotEmpty).toList();
  }
  ...
  Future<ChatCompletionResult> createCompletion({
    ...
    final response = await _dio.postUri(
      ...
      data: payload,
      options: Options(headers: _headers, sendTimeout: const Duration(seconds: 45), receiveTimeout: const Duration(seconds: 90)),
    );
    final data = response.data is String ? jsonDecode(response.data as String) : response.data;
    if (data is! Map) throw const FormatException('对话响应格式错误');
    final choices = (data['choices'] as List?) ?? const [];
    if (choices.isEmpty || choices.first is! Map) throw const FormatException('模型没有返回 choices');
