...
class McpConnectResult {
  const McpConnectResult({required this.protocolVersion, required this.serverName});
...
class McpClient {
  ...
  Future<McpConnectResult> connect() async {
    ...
    if (config.transport == McpTransport.streamableHttp) return _connectStreamable();
  ...
  Future<McpConnectResult> _connectStateless() async {
    ...
    return McpConnectResult(protocolVersion: currentProtocol, serverName: config.name);
  }
  ...
  Future<McpConnectResult> _connectDirect() async {
    ...
    final result = await _request('tools/list', const {});
    _toolCache = _parseTools(result);
    return McpConnectResult(protocolVersion: 'direct-jsonrpc', serverName: config.name);
  }
  ...
  Future<McpConnectResult> _connectLegacy() async {
    ...
    final info = await _request(
      'initialize',
      {
        ...
        'capabilities': const {},
        'clientInfo': {'name': 'TavoLink', 'version': '1.0.0', 'instanceId': _uuid.v4()},
      },
      initializing: true,
    );
    _protocolVersion = info['protocolVersion']?.toString() ?? latestLegacyProtocol;
  ...
  Future<Map<String, dynamic>> callTool(String name, Map<String, dynamic> arguments) {
    return _request('tools/call', {'name': name, 'arguments': arguments}, nameHeader: name);
  }
  ...
  List<McpTool> _parseTools(Map<String, dynamic> result) {
    ...
    return raw.whereType<Map>().map((e) => McpTool.fromJson(e.cast<String, dynamic>())).toList();
  }

  Map<String, String> _headers({required String method, String? nameHeader, bool initializing = false}) {
    ...
    if (token != null && token.trim().isNotEmpty) out['Authorization'] = 'Bearer ${token.trim()}';
    if (_mode == _WireMode.legacy && _sessionId != null) out['Mcp-Session-Id'] = _sessionId!;
    ...
    } else if (_mode == _WireMode.legacy && !initializing && _protocolVersion != null) {
  ...
  Future<Map<String, dynamic>> _request(
    ...
    final response = await _dio.postUri(
      config.url,
      data: {'jsonrpc': '2.0', 'id': id, 'method': method, 'params': requestParams},
      options: Options(
        headers: _headers(method: method, nameHeader: nameHeader, initializing: initializing),
    ...
    if (data['error'] != null) {
      ...
      throw McpException(error?['message']?.toString() ?? '未知 MCP 错误', error?['data']);
    }
    return (data['result'] as Map?)?.cast<String, dynamic>() ?? <String, dynamic>{};
  }
  ...
  String _encodeHeaderValue(String value) {
    final safeAscii = value.codeUnits.every((c) => c >= 0x20 && c <= 0x7e) && value.trim() == value;
