...
class McpConfigRepository {
  const McpConfigRepository({SecureStore secureStore = const SecureStore()}) : _secureStore = secureStore;
  ...
  Future<McpServerConfig?> load() async {
    ...
    final transport = McpTransport.values.where((e) => e.name == rawTransport).firstOrNull ?? McpTransport.auto;
    final headers = rawHeaders == null
        ? <String, String>{}
        : (jsonDecode(rawHeaders) as Map).map((k, v) => MapEntry(k.toString(), v.toString()));
    return McpServerConfig(name: name, url: uri, bearerToken: token, headers: headers, transport: transport);
  }
