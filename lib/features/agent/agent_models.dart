...
class AgentMessage {
  const AgentMessage({required this.role, required this.content, required this.createdAt});
  ...
  factory AgentMessage.fromJson(Map<String, dynamic> json) => AgentMessage(
        role: json['role'] == 'user' ? AgentMessageRole.user : AgentMessageRole.assistant,
...
class ToolActivity {
  ...
  ToolActivity copyWith({ToolActivityStatus? status, String? detail, int? elapsedMs}) => ToolActivity(
        id: id,
        name: name,
        label: label,
