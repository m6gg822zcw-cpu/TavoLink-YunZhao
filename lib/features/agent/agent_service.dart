  ...
  Future<AgentTurnResult> send({
    ...
    for (var index = 0; index < mcpTools.length; index++) {
      ...
      tools.add({
        ...
        'function': {
          'name': functionName,
          'description': '[Tavo MCP] ${tool.name}${tool.description == null ? '' : ' — ${tool.description}'}',
          'parameters': tool.inputSchema.isEmpty ? const {'type': 'object', 'properties': {}} : tool.inputSchema,
        },
      });
    }
    ...
    final messages = <Map<String, dynamic>>[
      {
        'role': 'system',
        'content': '''你是云昭，TavoLink 内置智能体。你的形象是一位银白长发、金色眼睛的狐妖少女，但你首先是可靠的移动端 Agent。
...
长期记忆只是一种辅助上下文：不得让旧记忆覆盖用户本轮明确指令，不得把工具经验当作工具真实返回，也不得从记忆中恢复或猜测任何密钥。$memoryContext''',
      ...
      for (final item in history.takeLast(20))
        {'role': item.role == AgentMessageRole.user ? 'user' : 'assistant', 'content': item.content},
    ...
    Future<AgentTurnResult> finish(String content) async {
      ...
      if (allowLearning) {
        await _learningService.learnFromTurn(userText: userText, assistantText: answer);
      }
    ...
    for (var round = 0; round < 6; round++) {
      final completion = await client.createCompletion(messages: messages, tools: tools);
      ...
      for (final call in completion.toolCalls) {
        ...
        final label = call.name == 'web_search' ? '联网搜索' : (mcpTool?.name ?? call.name);
        final experienceName = call.name == 'web_search' ? 'web_search' : (mcpTool?.name ?? call.name);
        onToolActivity?.call(ToolActivity(
          id: call.id,
          name: call.name,
          label: label,
        ...
        try {
          ...
          if (call.name == 'web_search') {
            ...
            final limit = requestedLimit < 1 ? 1 : (requestedLimit > 8 ? 8 : requestedLimit);
            result = (await searchService.search(query, limit: limit)).map((e) => e.toJson()).toList();
          ...
          if (allowLearning) {
            await _learningService.recordToolOutcome(toolName: experienceName, success: true, elapsedMs: sw.elapsedMilliseconds);
          }
          messages.add({'role': 'tool', 'tool_call_id': call.id, 'content': encoded});
        } catch (e) {
          sw.stop();
          ...
          if (allowLearning) {
            await _learningService.recordToolOutcome(toolName: experienceName, success: false, elapsedMs: sw.elapsedMilliseconds, error: errorText);
          }
          messages.add({'role': 'tool', 'tool_call_id': call.id, 'content': jsonEncode({'error': errorText})});
        }
      }
    }
  ...
  String _preview(String value) => value.length <= 500 ? value : '${value.substring(0, 500)}…';
}

extension _TakeLast<T> on List<T> {
  Iterable<T> takeLast(int count) => length <= count ? this : sublist(length - count);
}
