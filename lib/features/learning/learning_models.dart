...
class MemoryEntry {
  ...
  factory MemoryEntry.fromJson(Map<String, dynamic> json) => MemoryEntry(
        ...
        tags: (json['tags'] is List) ? (json['tags'] as List).map((e) => e.toString()).where((e) => e.isNotEmpty).toList() : const [],
        source: json['source']?.toString() ?? 'conversation',
      );

  static double _asDouble(Object? value, double fallback) => value is num ? value.toDouble() : double.tryParse(value?.toString() ?? '') ?? fallback;
  static int _asInt(Object? value, int fallback) => value is num ? value.toInt() : int.tryParse(value?.toString() ?? '') ?? fallback;
}
...
class LearningConfig {
  ...
  LearningConfig copyWith({
    ...
    int? maxMemories,
      ...
      LearningConfig(
        ...
        injectRelevantMemories: injectRelevantMemories ?? this.injectRelevantMemories,
  ...
  static int _readMax(Object? value) {
    final number = value is num ? value.toInt() : int.tryParse(value?.toString() ?? '') ?? 240;
...
class ToolExperience {
  ...
  int? get averageElapsedMs => totalCalls == 0 ? null : (totalElapsedMs / totalCalls).round();

  ToolExperience record({required bool success, required int elapsedMs, String? error}) => ToolExperience(
        toolName: toolName,
