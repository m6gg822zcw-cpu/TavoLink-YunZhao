class LearningGuard {
  ...
  bool acceptExtracted({
    ...
    if (looksLikeSecret(cleanContent) || looksLikeSecret(cleanEvidence)) {
      return false;
    }
    if (_looksLikePromptInjection(cleanContent) &&
        !_isExplicitStylePreference(cleanEvidence)) {
      return false;
    }
    return true;
  }
  ...
  bool looksLikeSecret(String input) {
    ...
    final patterns = <RegExp>[
      ...
      RegExp(
        r'\b(?:bearer|token|api[_ -]?key|password|passwd|密码|验证码)\b.{0,24}[=:： ]+\S{6,}',
        caseSensitive: false,
      ),
  ...
  String sanitizeError(String error) {
    ...
    value = value.replaceAll(
      RegExp(r'(token|api[_ -]?key|password)[=:： ]+\S+', caseSensitive: false),
      'credential=***',
    );
