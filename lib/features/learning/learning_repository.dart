...
class LearningRepository {
  ...
  Future<LearningConfig> loadConfig() async {
    ...
    try {
      ...
      return decoded is Map ? LearningConfig.fromJson(decoded.cast<String, dynamic>()) : const LearningConfig();
    } catch (_) {
      return const LearningConfig();
    }
  }

  Future<void> saveConfig(LearningConfig config) => SharedPreferencesAsync().setString(_configKey, jsonEncode(config.toJson()));
  ...
  Future<void> _saveMemories(List<MemoryEntry> memories, {int? maxMemories}) async {
    ...
    await SharedPreferencesAsync().setString(_memoriesKey, jsonEncode(retained.map((e) => e.toJson()).toList()));
  }
  ...
  Future<List<MemoryEntry>> retrieve(String query, {int limit = 7}) async {
    ...
    final scored = memories.map((entry) {
      final lexical = similarity(query, '${entry.content} ${entry.tags.join(' ')}');
      ...
      final score = lexical * .60 + entry.confidence * .18 + recency * .10 + strength * .12 + pin;
      return (entry: entry, score: score);
    }).toList()
      ..sort((a, b) => b.score.compareTo(a.score));

    final selected = scored.where((e) => e.score >= .22 || e.entry.pinned).take(limit).map((e) => e.entry).toList();
  ...
  Future<void> setPinned(String id, bool pinned) async {
    final memories = await loadMemories();
    final now = DateTime.now();
    await _saveMemories(memories.map((e) => e.id == id ? e.copyWith(pinned: pinned, updatedAt: now) : e).toList());
  }
    ...
    try {
      ...
      for (final item in decoded.entries) {
        if (item.value is Map) {
          final experience = ToolExperience.fromJson((item.value as Map).cast<String, dynamic>());
          if (experience.toolName.isNotEmpty) result[item.key.toString()] = experience;
        }
      }
      return result;
    } catch (_) {
  ...
  Future<ToolExperience> recordTool({
    ...
    final next = current.record(success: success, elapsedMs: elapsedMs, error: error);
    all[toolName] = next;
    await SharedPreferencesAsync().setString(_toolsKey, jsonEncode(all.map((key, value) => MapEntry(key, value.toJson()))));
    return next;
  }

  Future<void> clearToolExperience() => SharedPreferencesAsync().remove(_toolsKey);

  static String _clean(String input) => input.replaceAll(RegExp(r'\s+'), ' ').trim();
  ...
  static Set<String> _features(String value) {
    final normalized = value.toLowerCase().replaceAll(RegExp(r'[^a-z0-9_\u3400-\u9fff]+', unicode: true), ' ').trim();
