...
class _ChatPageState extends State<ChatPage> {
      ...
      if (messages.isEmpty) {
        messages.add(
          AgentMessage(
            ...
            content:
                '我是云昭。配置好模型 API 后，我可以陪你聊天，也能通过 Tavo MCP 调用工具、通过联网搜索查找实时信息。',
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      if (!mounted) return;
      final prefix = result.warnings.isEmpty
          ? ''
          ...
          children: [
            ...
            Flexible(
              child: Container(
                ...
                decoration: BoxDecoration(
                  gradient: user
                      ? const LinearGradient(
                          colors: [Color(0xFF6C4FE7), Color(0xFF5378F6)],
                        )
                      : LinearGradient(
                          colors: [
                            TavoPalette.panelSoft.withValues(alpha: .92),
                            TavoPalette.navy.withValues(alpha: .88),
                          ],
                        ),
                  ...
                  boxShadow: [
                    BoxShadow(
                      color: (user ? TavoPalette.violet : Colors.black)
                          .withValues(alpha: .16),
                      blurRadius: 18,
                ...
                child: Text(
                  message.content,
                  style: const TextStyle(height: 1.52),
                ),
              ),
            ),
          ],
    ...
    return const Padding(
      ...
      child: Row(
        children: [
          ...
          Text(
            '狐火正在流转…',
            style: TextStyle(color: TavoPalette.muted, fontSize: 12),
          ),
        ],
      ),
    );
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(14),
            border: Border.all(
              color: color.withValues(alpha: enabled ? .30 : .12),
            ),
          ),
          child: Row(
